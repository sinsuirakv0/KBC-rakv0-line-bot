...
export const botCommand: LineCommand = {
	...
	async execute(command) {
		...
		if (action !== "status") {
			await message.send("使い方: !bot [status|start|stop|admin|setting|log]\n詳しくは !bot help");
			return;
		}
		...
		await message.send([
			...
			`LINE送信: 実行中 ${lineApi.activeCount}/${lineApi.maxConcurrent} / 待機 ${lineApi.pendingCritical + lineApi.pendingHigh + lineApi.pendingNormal} (通知 ${lineApi.pendingCritical})${lineApi.active ? ` / 最長 ${formatDuration(lineApi.oldestActiveMs)} ${lineApi.active}` : ""}`,
		].join("\n"));
	},
};
